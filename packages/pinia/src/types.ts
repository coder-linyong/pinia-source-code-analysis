...
/**
 * 选项存储的 “definestore ()” 的选项参数。可以通过插件API扩展到扩充商店。
 * @see {@link DefineStoreOptionsBase}.
 */
...
> extends DefineStoreOptionsBase<S, Store<Id, S, G, A>> {
  /**
   * 唯一的字符串键，用于标识跨应用程序的存储。
   */
  id: Id

  /**
   * 函数来创建一个新的状态。必须是箭头功能，以确保正确的打字!
   */
  state?: () => S
  ...
  /**
   * 当在存储定义中使用复杂状态 (例如仅客户端参考) 并且从 “pinia.state' 复制值是不够的时，允许在SSR期间对存储进行水合。
   *
   * @example
   * 如果在您的 “状态” 中，您使用在服务器和客户端上具有不同值的任何 'customref'，
   * 任何 'computed' 或任何 'ref'，则需要手动补充它们。例如，存储在本地存储中的自定义ref:
   *
   * ```ts
